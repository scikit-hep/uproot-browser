...
- **`src/uproot_browser/plotext_compat.py`** — version adaptor: all plotext drawing goes through the `PlotextFigure` protocol, with implementations for plotext 5.2.8+ (module-level API, no 2D heatmaps — raises a clear error) and 6+ (figure-object API). The `minimums` nox session exercises the 5.x path.
